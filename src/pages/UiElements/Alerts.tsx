...
const Alert: React.FC<AlertProps> = ({ type, title, message, onClose }) => {
  ...
  return (
    <div
      className={`flex w-full border-l-6 ${currentStyle.borderColor} ${currentStyle.bgColor} px-7 py-8 shadow-md dark:bg-[#1B1B24] dark:bg-opacity-30 md:p-9`}
    >
      <div
        className={`mr-5 flex h-9 w-9 items-center justify-center rounded-lg ${currentStyle.iconBg}`}
        onClick={onClose}
      >
      ...
      <div className="w-full">
        <h5 className={`mb-3 text-lg font-semibold ${currentStyle.textColor}`}>
          {title}
        </h5>
        <p className={`text-base leading-relaxed ${currentStyle.messageColor}`}>
          {message}
        </p>
      </div>
    </div>
